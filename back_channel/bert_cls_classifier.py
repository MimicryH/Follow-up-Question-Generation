...
from FollowUps.back_channel.tokenizer import load_chinese_base_vocab, Tokenizer
...
class BertClsClassifier(nn.Module):
    ...
    def __init__(self, word2ix, target_size, model_name="roberta"):
        ...
        if model_name == "roberta":
            from FollowUps.back_channel.model.roberta_model import BertModel, BertConfig
            config = BertConfig(len(self.word2ix))
            self.bert = BertModel(config)
        elif model_name == "bert":
            from FollowUps.back_channel.model.bert_model import BertConfig, BertModel
            config = BertConfig(len(self.word2ix))
            self.bert = BertModel(config)
        else:
    ...
    def compute_loss(self, predictions, labels):
        ...
        # print("compute_loss: ", predictions.size(), labels.size())
